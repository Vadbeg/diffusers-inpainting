...
class StableDiffusionSimpleInpaintingPipeline(
    ...
    def _encode_prompt(  # type: ignore
        self,
        prompt,
        device,
    ...
    def __call__(  # type: ignore
        ...
        Args:
            ...
            image (`torch.FloatTensor`, `PIL.Image.Image`, `np.ndarray`, `List[torch.FloatTensor]`, `List[PIL.Image.Image]`, or `List[np.ndarray]`):
                ...
                process. Can also accpet image latents_with_noise as `image`, if passing latents_with_noise directly, it will not be encoded
                again.
            ...
            callback (`Callable`, *optional*):
                ...
                called with the following arguments: `callback(step: int, timestep: int, latents_with_noise: torch.FloatTensor)`.
        ...
        latents_with_noise, init_latents, noise = self.prepare_latents(
            image,
            latent_timestep,
            batch_size,
            num_images_per_prompt,
        ...
        mask = mask.to(device)
        mask = mask.to(prompt_embeds.dtype)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                # expand the latents_with_noise if we are doing classifier free guidance
                latent_model_input = (
                    torch.cat([latents_with_noise] * 2)
                    if do_classifier_free_guidance
                ...
                latents_with_noise = self.scheduler.step(
                    noise_pred,
                    t,
                    latents_with_noise,
                    **extra_step_kwargs,
                ...
                if i == len(timesteps) - 1 or (
                    ...
                    if callback is not None and i % callback_steps == 0:
                        callback(i, t, latents_with_noise)

        if not output_type == "latent":
            image = self.vae.decode(
                latents_with_noise / self.vae.config.scaling_factor, return_dict=False
            )[0]
            image, has_nsfw_concept = self.run_safety_checker(
                image, device, prompt_embeds.dtype
            )
        else:
            image = latents_with_noise
            has_nsfw_concept = None
